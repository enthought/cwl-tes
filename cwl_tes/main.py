...
def s3_upload(base_url, fs_access, cwl_obj):
...
def main(args=None):
    ...
    s3_fs_access = S3FsAccess(os.curdir, url=endpoint, insecure=insecure)
    bucket = s3_fs_access.join(bucket, str(uuid.uuid4()))
    parsed_args.remote_storage_url = bucket
    ...
    runtime_context.path_mapper = functools.partial(
        TESPathMapper, fs_access=s3_fs_access)
...
def tes_execute(process,           # type: Process
    ...
    if remote_storage_url:
        upload_workflow_deps_s3(process, remote_storage_url, s3_access)
        ...
        job_order = upload_job_order_s3(
            process, job_order, remote_storage_url, s3_access)
...
def upload_workflow_deps_s3(process, remote_storage_url, s3_access):
    """
    ...
    def upload_tool_deps(deptool):
        if "id" in deptool:
            upload_dependencies_s3(document_loader, deptool, deptool["id"],
                                   True, remote_storage_url, s3_access)
            document_loader.idx[deptool["id"]] = deptool
    process.visit(upload_tool_deps)


def upload_dependencies_s3(document_loader, workflowobj, uri, loadref_run,
                           remote_storage_url, s3_access):
    """
    Upload the dependencies of the workflowobj document to an S3 location.
    ...
    def visit_default(obj):
        ...
        def ensure_default_location(fileobj):
            ...
            if "location" in fileobj \
                    and not s3_access.exists(fileobj["location"]):
    ...
    visit_class(workflowobj, ("Directory"), functools.partial(
        s3_upload, remote_storage_url, s3_access))
...
def upload_job_order_s3(process, job_order, remote_storage_url, ftp_access):
    """
    ...
    upload_dependencies_s3(process.doc_loader, job_order,
